...
tray_init(void)
{
	...
	/* Reload an item's icon when it signals a change (also catches icons
	 * that weren't ready at the initial RegisterStatusNotifierItem). */
	...
	sd_bus_add_match(tray_bus, NULL,
		...
		tray_item_new_icon, NULL);
	sd_bus_add_match(tray_bus, NULL,
		"type='signal',interface='org.freedesktop.StatusNotifierItem',member='NewIcon'",
		tray_item_new_icon, NULL);
	sd_bus_add_match(tray_bus, NULL,
