 ...
 * buffers in a tree on the overlay layer, centered on selmon.
 */
...
#define GUIDE_PAD_X    36
#define GUIDE_PAD_Y    26
#define GUIDE_ROW_H    54
#define GUIDE_RADIUS   16
#define GUIDE_MIN_W    380
...
guide_card_buffer(int w, int h, int sel)
{
	...
	guide_rounded_rect(cr, 0.5, 0.5, w - 1.0, h - 1.0, GUIDE_RADIUS);
	...
	guide_rounded_rect(cr,
			GUIDE_PAD_X / 2.0,
			GUIDE_PAD_Y + (double)sel * GUIDE_ROW_H,
			w - GUIDE_PAD_X, GUIDE_ROW_H, 10);
...
guide_text_width(const char *s)
{
	...
	for (size_t i = 0; s[i]; i++) {
		...
		if (prev_cp)
			fcft_kerning(statusfont.font, prev_cp, cp, &kx, &ky);
		pen_x += (int)kx;
...
guide_menu_build(void)
{
	...
	card_w = GUIDE_MIN_W;
	for (i = 0; i < GUIDE_N; i++) {
		w = guide_text_width(entries[i]) + GUIDE_PAD_X * 3;
		if (w > card_w)
			card_w = w;
	}
	card_h = GUIDE_PAD_Y * 2 + GUIDE_ROW_H * GUIDE_N;
	...
	text_h = statusfont.font->ascent + statusfont.font->descent;
	for (i = 0; i < GUIDE_N; i++)
		guide_draw_text(menu_tree, entries[i],
				GUIDE_PAD_X + GUIDE_PAD_X / 2,
				GUIDE_PAD_Y + i * GUIDE_ROW_H
					+ (GUIDE_ROW_H - text_h) / 2
					+ statusfont.font->ascent);
}
