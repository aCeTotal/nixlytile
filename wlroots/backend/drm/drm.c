...
bool check_drm_features(struct wlr_drm_backend *drm) {
	...
	/* Always probe the raw kernel capability -- needed in get_fb_for_bo()
	 * even when WLR_DRM_NO_MODIFIERS suppresses modifier negotiation.
	 * Nvidia requires DRM_MODE_FB_MODIFIERS for implicit modifiers. */
	{
		int ret = drmGetCap(drm->fd, DRM_CAP_ADDFB2_MODIFIERS, &cap);
		drm->cap_addfb2_modifiers = ret == 0 && cap == 1;
	}

	if (env_parse_bool("WLR_DRM_NO_MODIFIERS")) {
		wlr_log(WLR_DEBUG, "WLR_DRM_NO_MODIFIERS set, disabling modifiers");
	} else {
		drm->addfb2_modifiers = drm->cap_addfb2_modifiers;
